...
public class UnrepeatableReadSim {
	...
	public void startSim() { // The method in which all of the simulation
		...
		try {
			...
			// Execute the select query for the first time to create a resultset
			// to compare with
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					for (int i = 1; i <= 3; i++) {
						int rnnb = rn.nextInt(10); // Get the random mutation
													// number
						int np = rn.nextInt(2); // Get a random number
						switch (np) { // Determine if the random number wil
										// create a positive or a negative
										// mutation
							case 0:
								int rnnbTemp = rnnb;
						...
						try {
							...
							transactionA.executeUpdate(); // Execute the insert
															// into mutations
															// query
							ResultSet rsRA = db.stmt.executeQuery(sqlReadAmount); // Get
																					// the
																					// results
																					// of
																					// the
							...
							while (rsRA.next()) {
								stAmount = rsRA.getInt("st_amount"); // Save the
																		// result
																		// in a
																		// variable
								System.out.println("Total amount before:" + stAmount);
							}
							ResultSet rsTA = db.stmt.executeQuery(sqlTotalAmount); // Get
																					// the
																					// results
																					// of
																					// the
																					...
																					// made
							while (rsTA.next()) {
								totalAmount = rsTA.getInt("total"); // Save the
																	// result in
																	// a
																	// variable
							}
							...
							String sqlUpdateAmount = "UPDATE stocks SET st_amount = '" + totalAmount
									+ "' WHERE p_id = 1"; // prepare the query
															// for updating the
															// stock
							transactionA = db.conn.prepareStatement(sqlUpdateAmount);
							transactionA.executeUpdate(); // Run the stock
															// update
			...
			}, "Insert thread").start(); // Finish defining the thread and start
											// the thread
