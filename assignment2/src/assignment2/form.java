...
public class form {
	public static void main(String[] args) {
		Frame frm = new Frame("Select a Simulation"); // Create the frame for the
													// form
		frm.setSize(350, 200); // Set the size of the frame
		frm.setVisible(true); // Define if the frame should be visible
		frm.addWindowListener(new WindowAdapter() { // Make sure the window can
													// be closed by the cross
													// button
			@Override
		...
		Panel p = new Panel(); // Create the panel in which the form will be
								// placed
		Panel p1 = new Panel(); // Create the panel in which the previous panel
								// will be placed to give a little padding in
								// the frame

		final CheckboxGroup sim = new CheckboxGroup(); // Create the group for
														// the radio boxes
